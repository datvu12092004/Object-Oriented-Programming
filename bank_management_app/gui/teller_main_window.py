...
class TellerMainWindow(QWidget):
    def __init__(self, nhan_vien):
        ...
        # ===== EVENTS =====
        btnDeposit.clicked.connect(
            lambda: self.load_widget(DepositGUI(self.nv))
        )
        btnWithdraw.clicked.connect(
            lambda: self.load_widget(WithdrawGUI(self.nv))
        )
        btnTransfer.clicked.connect(
            lambda: self.load_widget(TransferGUI(self.nv))
        )
        btnHistory.clicked.connect(
            lambda: self.load_widget(HistoryGUI())
        )
        btnSaving.clicked.connect(
            lambda: self.load_widget(SavingGUI(self.nv))
        )

        btnAddKH.clicked.connect(
            lambda: self.load_widget(CustomerCreateGUI())
        )
        btnSearchKH.clicked.connect(
            lambda: self.load_widget(CustomerSearchGUI())
        )
    ...
    def logout(self):
        """
        Đăng xuất nhân viên:
        đóng cửa sổ hiện tại và quay về màn hình chọn vai trò.
        Sử dụng lazy import để tránh circular import.
        """
        from gui.role_select_gui import RoleSelectGUI  # lazy import
        self.roleSelect = RoleSelectGUI()
        self.roleSelect.show()
        self.close()
