...
class DepositGUI(QWidget):
    ...
    def do_deposit(self):
        ...
        # 2. Gọi service
        ok, result = deposit(so_tk, so_tien, self.nv[0])

        if not ok:
            QMessageBox.warning(self, "Không thể nộp tiền", result)
            return

        # 3. Thành công
        QMessageBox.information(self, "Thành công", "Nộp tiền thành công")
